...
def suggest_prompts(df: pd.DataFrame, max_suggestions: int = 8):
    ...
    if categorical:
        col = categorical[0]
        suggestions.append(
            f"Show the top 10 counts for the categorical column '{col}'."
        )
    # Numeric summaries
    if numeric:
        suggestions.append(
...
def prompt_to_code(prompt: str, df: pd.DataFrame):
    ...
    if p.startswith("summarize the dataset"):
        code = textwrap.dedent(
            """
            # produce a beautiful, clean vertical plain-text layout
            total_rows = len(df)
            total_cols = len(df.columns)
            numeric_cols = df.select_dtypes(include=['number']).columns.tolist()

            # Format Column Schema on separate lines with clean spacing
            col_types_list = [f"  • {c} ({str(df[c].dtype)})" for c in df.columns]
            col_types_str = "\\n".join(col_types_list)
            ...
            if len(missing) > 0:
                missing_str = "\\n".join([f"  • {idx}: {val} missing values" for idx, val in missing.items()])
            else:
                missing_str = "  • None! Your dataset is perfectly clean."

            # Build a structured plain text block with physical line breaks
            result = (
                f"📊 DATASET OVERVIEW\\n"
                f"==================\\n"
                ...
                f"⚙️ DATA SCHEMA (COLUMNS)\\n"
                f"=======================\\n"
                f"{col_types_str}\\n\\n"
                f"⚠️ MISSING DATA ALERT\\n"
                f"====================\\n"
                f"{missing_str}"
            )
            """
    ...
    if (
        "top 10 counts for the categorical column" in p
        or "top 10 counts" in p
        and "'" in p
    ):
    ...
    # Histogram
    if (
        p.startswith("create a histogram of the numeric column")
        or "histogram of the numeric column" in p
    ):
        import re
        ...
        if col:
            ...
            if "monthly sum" in p and "using the datetime column" in p:
                ...
                m = re.search(
                    r"sum of '([^']+)' using the datetime column '([^']+)'", prompt
                )
                if m:
...
def run_code(df: pd.DataFrame, code: str):
    ...
    try:
        # run
        # Replace it with this updated line:
        ...
        return {
            "type": "text",
            "output": "Execution finished. No result produced.",
        }
